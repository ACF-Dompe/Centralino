...
| **9.2** | Errori di connessione al WLC tutti riportati come "Host irraggiungibile" | ✅ **FIXED** | `req.on(error)` etichettava ogni fallimento — rete **e** TLS — come `Host irraggiungibile`. Col certificato self-signed del Catalyst 9800 rifiutato da `WLC_TLS_REJECT_UNAUTHORIZED=true`, l operatore è stato mandato a indagare egress, routing e rete del container mentre il controller rispondeva regolarmente. Ora l errore è classificato (certificato rifiutato/scaduto, connessione rifiutata, host non risolto, timeout reale, generico) con messaggi azionabili che nominano la variabile da cambiare; la classificazione usa `err.code` con fallback sul messaggio. Aggiunto `code` alla riga di log. Test: `wlcWebui.test.ts`. |
...
| **5.14** | Convenzione amministratori valutata sull'indirizzo di posta: gli account amministrativi non erano riconosciuti | ✅ **FIXED** | La regola `RBAC_AUTO_ADMIN_PREFIXES` / `RBAC_AUTO_ADMIN_DOMAINS` — l'unica via per raggiungere un deployment nuovo senza break-glass — si valutava sulla claim `emailaddress`. Ma un account amministrativo non ha casella: `admin365-…@dompe.onmicrosoft.com` non ne ha in questo tenant, quindi la claim arriva vuota e la convenzione mancava **esattamente** gli account per cui esiste. In produzione l'amministratore ha completato l'SSO ed è finito su *Accesso in attesa di abilitazione*, senza nessun altro account già abilitato che potesse sbloccarlo: la via d'uscita restava solo la CLI. La convenzione si valuta ora sullo **UPN**, letto in ordine da `.../claims/upn`, da `.../claims/name` se ha forma di indirizzo (default di Entra, `user.userprincipalname`) e dal `NameID`; l'indirizzo di posta resta come fallback solo quando nessuna claim porta un UPN, così le installazioni esistenti non cambiano comportamento. **La chiave di `app_users` resta l'`objectId` Entra**: `upn` è una colonna nuova, indicizzata su `LOWER(upn)` ma deliberatamente **non univoca e non identificante** — un rename dell'utente aggiorna la colonna invece di creare un secondo profilo, e l'identità non dipende da un valore riassegnabile. Il vincolo sul dominio è inalterato e continua a fare il lavoro che conta: senza di esso un guest B2B `admin365-x@attacker.com` si autopromuoverebbe amministratore, e una lista di domini vuota disabilita la regola anziché aprirla. L'UPN è anche ciò che il frontend mostra all'utente in attesa, al posto di una mail vuota. Test: `samlUpn.test.ts`, `appUsers.test.ts`. |
...
| **P1** | **Convenzione amministratori spostata su UPN (la mail non esiste sugli account amministrativi); chiave `app_users` invariata sull'`objectId`** | 5.14 | *questa sessione* |

...
export function createAuthRouter(opts) {
    ...
    router.get('/login', (req, res, next) => {
        ...
        req.session.samlRedirect = redirectTo;
    ...
    router.post('/callback', (req, res, next) => {
        passport.authenticate('saml', {
            ...
            const redirectTo = sessionData.samlRedirect || '/';
    ...
    router.post('/logout', (req, res, next) => {
        ...
        buildSloRedirectUrl(samlStrategy, req)
            ...
            req.logout(() => {
                req.session.destroy(() => {
                    res.clearCookie('cgd.sid');
                    log.info({ nameID: user.nameID, email: user.email }, 'Local session destroyed, redirecting to IdP SLO endpoint');
            ...
            log.warn({ err: err.message, nameID: user.nameID }, 'SLO initiation failed, falling back to local logout');
            req.logout(() => {
                req.session.destroy(() => {
                    res.clearCookie('cgd.sid');
     ...
     */
    router.post('/slo/callback', passport.authenticate('saml', {
        successRedirect: '/',

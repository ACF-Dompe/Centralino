...
export async function listGuests(filter) {
    ...
    const sql = `SELECT * FROM guests` + (where.length ? ` WHERE ${where.join(' AND ')}` : '') + ` ORDER BY created_at DESC`;
...
export async function createGuest(g) {
    const db = await getDb();
    const status = g.status ?? 'pending';
    await db.query(`INSERT INTO guests
...
export async function getSedeByCode(code) {
    const db = await getDb();
 ...
 */
export async function getWlcConfigBySede(sedeId) {
    const db = await getDb();
    let res;
    if (sedeId != null) {
        res = await db.query(`SELECT * FROM wlc_config WHERE sede_id = ? OR id = (SELECT wlc_config_id FROM sedi WHERE id = ?) ORDER BY (sede_id = ?) DESC LIMIT 1`, [sedeId, sedeId, sedeId]);
 ...
 */
export async function getWlcConfig() {
    return getWlcConfigBySede(null);
}
export async function updateWlcConfigBySede(sedeId, patch) {
    const db = await getDb();
    const map = {
        host: 'host',
        port: 'port',
    ...
    if (sets.length > 0) {
        await db.query(`UPDATE wlc_config SET ${sets.join(', ')}
       WHERE sede_id = ? OR id = (SELECT wlc_config_id FROM sedi WHERE id = ?)`, [...params, sedeId, sedeId]);
    }
 ...
 */
export async function updateWlcConfig(patch) {
    const db = await getDb();
    const map = {
        host: 'host',
        port: 'port',
...
export async function getSmsConfig() {
    ...
    return {
        id: 1,
        gatewayType: r.gateway_type ?? 'textbelt',
        apiKey: r.api_key ?? null,
        senderId: r.sender_id ?? 'DompeGuest',
...
export async function listSyncLogs(limit = 100) {
    ...
    const res = await db.query(`SELECT * FROM sync_logs ORDER BY timestamp DESC LIMIT ?`, [limit]);
...
export async function addSyncLog(entry) {
    const db = await getDb();
    await db.query(`INSERT INTO sync_logs (timestamp, action, method, url, payload, status_code) VALUES (CURRENT_TIMESTAMP, ?, ?, ?, ?, ?)`, [entry.action, entry.method, entry.url ?? null, entry.payload ?? null, entry.statusCode ?? null]);
}
